        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    # # Print tool calls and responses with 10% probability for debugging
                    ...
                    #             resp_str = resp.text if resp.text else str(resp)
                    ...
                    #         print(
                    #             f"[Tool Call] Request ID: {_req.request_id}, Tool: {tool_call.function.name}, Arguments: {args_str}"
                    #         )
                    #         print(
                    #             f"[Tool Response] Request ID: {_req.request_id}, Tool: {tool_call.function.name}, Response: {resp_str}"
                    #         )
                    #         # Debug: check if resp is ToolResponse and has text
                    #         if hasattr(resp, 'text'):
                    #             print(
                    #                 f"[Tool Response Debug] Request ID: {_req.request_id}, Tool: {tool_call.function.name}, "
                    #                 f"resp type: {type(resp)}, resp.text type: {type(resp.text)}, "
                    #                 f"resp.text is None: {resp.text is None}, resp.text length: {len(resp.text) if resp.text else 0}"
                    #             )
                    # # Debug: check tool responses before adding to messages
                    # if random.random() < 0.1:
                    #     print(f"[Before add_tool_response_messages] Request ID: {_req.request_id}")
                    #     for i, (resp, _, _) in enumerate(tool_call_results):
                    #         if hasattr(resp, 'text'):
                    #             print(
                    #                 f"  Tool response {i}: type={type(resp)}, text type={type(resp.text)}, "
                    #                 f"text is None={resp.text is None}, text length={len(resp.text) if resp.text else 0}, "
                    #                 f"text preview={repr(resp.text[:100]) if resp.text else 'None'}"
                    #             )
                    #         else:
                    #             print(f"  Tool response {i}: type={type(resp)}, value={repr(str(resp)[:100])}")
                    
                    _req.add_tool_response_messages(
                        self.processing_class, 
                        [resp for resp, _, _ in tool_call_results],
                        max_content_tokens=self.config.multi_turn.max_tool_response_tokens
                    )
                    # Debug: check messages after adding tool response
                    if random.random() < 0.1:
                        print(f"[After add_tool_response_messages] Request ID: {_req.request_id}, max_content_tokens: {self.config.multi_turn.max_tool_response_tokens}")
                        # Check the last few messages
                        for i, msg in enumerate(_req.messages[-len(tool_call_results):]):
                            print(
                                f"  Message {i}: role={msg.role}, content type={type(msg.content)}, "
                                f"content length={len(msg.content) if isinstance(msg.content, list) else 'N/A'}"
                            )
                            if isinstance(msg.content, list):
                                for j, content_item in enumerate(msg.content):
                                    if isinstance(content_item, dict):
                                        print(
                                            f"    Content item {j}: type={content_item.get('type')}, "
                                            f"text length={len(content_item.get('text', '')) if content_item.get('text') else 0}, "
                                            f"text preview={repr(content_item.get('text', '')[:100]) if content_item.get('text') else 'None'}"
                                        )
                    for tool_call, (resp, reward, metrics) in zip(parsed_tool_calls, tool_call_results, strict=True):
